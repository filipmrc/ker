...
public:
  KerPositionInterface(ros::NodeHandle n)
 {
  ...
  void initInterface()
  {
    ...
    hardware_interface::JointHandle pos_handle_1(jnt_state_interface.getHandle("shoulder_1_joint"), &cmd[0]);
    jnt_pos_interface.registerHandle(pos_handle_1);

    hardware_interface::JointHandle pos_handle_2(jnt_state_interface.getHandle("humerus_1_joint"), &cmd[1]);
    ...
    hardware_interface::JointHandle pos_handle_4(jnt_state_interface.getHandle("shoulder_2_joint"), &cmd[3]);
    jnt_pos_interface.registerHandle(pos_handle_4);

    hardware_interface::JointHandle pos_handle_5(jnt_state_interface.getHandle("humerus_2_joint"), &cmd[4]);
    jnt_pos_interface.registerHandle(pos_handle_5);

    hardware_interface::JointHandle pos_handle_6(jnt_state_interface.getHandle("radius_2_joint"), &cmd[5]);
    jnt_pos_interface.registerHandle(pos_handle_6);

    hardware_interface::JointHandle pos_handle_7(jnt_state_interface.getHandle("shoulder_3_joint"), &cmd[6]);
    jnt_pos_interface.registerHandle(pos_handle_7);

    hardware_interface::JointHandle pos_handle_8(jnt_state_interface.getHandle("femur_2_joint"), &cmd[7]);
    jnt_pos_interface.registerHandle(pos_handle_8);

    hardware_interface::JointHandle pos_handle_9(jnt_state_interface.getHandle("tibula_2_joint"), &cmd[8]);
    jnt_pos_interface.registerHandle(pos_handle_9);

    hardware_interface::JointHandle pos_handle_10(jnt_state_interface.getHandle("shoulder_4_joint"), &cmd[9]);
    jnt_pos_interface.registerHandle(pos_handle_10);

    hardware_interface::JointHandle pos_handle_11(jnt_state_interface.getHandle("femur_1_joint"), &cmd[10]);
    jnt_pos_interface.registerHandle(pos_handle_11);

    hardware_interface::JointHandle pos_handle_12(jnt_state_interface.getHandle("tibula_1_joint"), &cmd[11]);
    jnt_pos_interface.registerHandle(pos_handle_12);
  ...
  void setJointPositions(ros::Duration period)
  {
    ...
    for(int i = 0; i<NUM_JOINTS_LEG; i++)
      {
	tmp = ((cmd[i] + PI*150/180)*(1023*180)/(300*PI));
	strs << tmp;
      }
    strs << endl;
